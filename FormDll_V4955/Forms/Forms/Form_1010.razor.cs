	...
	public class Form_1010Base : Form_1010Peropeties
	{
		...
		protected override async Task OnAfterRenderAsync(bool firstRender)
		{
			if (firstRender)
			{
				...
				// در ابتدا هر دو کنترل بررسی کد رهگیری و داده‌های همه مخفی باشند
				// بر اساس مقدار اولیه IsResultOfAnotherProcess تنظیم می‌شود
		...
		private async Task<bool> CheckFieldsValidation(Entity.IDMS_RDC_Details Item)
		{
			...
			{
				IsValid = false;
				await _MSG.ShowWarning("لطفاً دسته‌بندی محصول را انتخاب کنید.");
			}
			if (Item.IDMS_ProductsId == null)
			{
				IsValid = false;
				await _MSG.ShowWarning("لطفاً محصول را انتخاب کنید.");
			}
			if (Item.IDMS_CustomerId == null)
			{
				IsValid = false;
				await _MSG.ShowWarning("لطفاً مشتری را انتخاب کنید.");
			}
			if (Item.IDMS_ResultingFromId == null)
			{
				IsValid = false;
				await _MSG.ShowWarning("لطفاً گزینه «منتج از» را انتخاب کنید.");
			}
		...
		public async Task IsResultOfAnotherProcess_oninput(ChangeEventArgs selected)
		{
			...
			Ref_TrackingCode?.SetVisible(isChecked);
			Ref_IDMS_RDC_AllData?.SetVisible(isChecked);

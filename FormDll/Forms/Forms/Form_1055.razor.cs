...
{
    public class Form_1055Base : Form_1055Peropeties
    {
    ...
    public async Task  submit_onclick(MouseEventArgs Selected   )
        {
            ...
            {
                toastService.ShowError("لطفا گزینه سال را  به صورت صحیح وارد کنید ( مثال : 1404 )",
                    settings =>
                    {
                        settings.Timeout = 4;
            ...
            if (_Entity.Month == null)
            {
                toastService.ShowError("لطفا گزینه ماه را  به صورت صحیح وارد کنید ( مثال : 9 )",
                    settings =>
                    {
                        settings.Timeout = 4;
                        settings.ShowProgressBar = true;

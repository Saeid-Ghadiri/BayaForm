...
{
    public class Form_692Base : Form_692Peropeties
    {
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            ...
            _Entity.SCM_ProductRequestDetails = _Entity.SCM_ProductRequestDetails.Where(p=>p.IsDelete == false).ToList();        
			StateHasChanged();
        }
    }
    ...
    public override async Task<bool> FormValidator()
    {
       bool IsValid = true;
        ...
        var List = _Entity.SCM_ProductRequestDetails.Where(p=>p.IsDelete == false).ToList();
            ...
            // دکمه ثبت و ادامه           
            if (BtnWorkFlowId == "SequenceFlow_1f1nupt"||BtnWorkFlowId == "SequenceFlow_16dlp30" )
            {
    ...
    /// <summary>
        ...
        public async Task<bool> CheckFieldValidation(Entity.SCM_ProductRequestDetails Item)
        {
           ...
           // Note: شرط پُر بودن فیلد نام کالا دیزیبل
			...
			if (Item.FutureActionTrueFalse == null)
			{
				IsValid = false;

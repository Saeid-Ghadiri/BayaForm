...
{
	public class Form_1050Base : Form_1050Peropeties
	{
		...
		{
			foreach(var item in _Entity.SCMNFP_ProductRequestDetails){
		...
		public async Task<bool> CheckFieldValidation(Entity.SCMNFP_ProductRequestDetails Item)
		{
			...
			// - نام کالا
			if (Item.ProductExist == null)
			{
				IsValid = false;

				toastService.ShowError("لطفا گزینه موجودی کالا را تکمیل نمایید",
					settings =>
					{
						settings.Timeout = 4;

...
{
	public class Form_1114Base : Form_1114Peropeties
	{
		...
		private async Task GenerateUniqueDeliveryCodeForAllRows()
		{
			...
			} while (_Entity.SCMATLASCELL_ProductRequestDetails.Any(x => x.DeliveryCode == newCode && x.IsDelete != true));
		...
		{
			Ref_SCMATLASCELL_ProductRequestDetails_ResultingFromITIL.SetVisible(Visible);
		}
		public async Task ITILDetailsVisible(bool Visible)
		{
			Ref_SCMATLASCELL_ProductRequestDetails_RequestIdITIL.SetVisible(Visible);
			Ref_SCMATLASCELL_ProductRequestDetails_RequestIdITIL.SetDisabled(true);
			Ref_SCMATLASCELL_ProductRequestDetails_RequesterUserITIL.SetVisible(Visible);
			Ref_SCMATLASCELL_ProductRequestDetails_RequesterUserITIL.SetDisabled(true);

			Ref_SCMATLASCELL_ProductRequestDetails_CreatedAtITIL.SetVisible(Visible);
			Ref_SCMATLASCELL_ProductRequestDetails_CreatedAtITIL.SetDisabled(true);

			Ref_SCMATLASCELL_ProductRequestDetails_ITILDetails.SetVisible(Visible);
		}

		#endregion

		#region مدیریت شرایط تحویل و خرید
		public async Task TahvilIsVisible(bool Visible, bool Value, Entity.SCMATLASCELL_ProductRequestDetails Item)
		{
			// تحویل
			Ref_SCMATLASCELL_ProductRequestDetails_GoodsDeliveryIsEnable.SetVisible(Visible);
			Item.GoodsDeliveryIsEnable = Value;
		}

		public async Task KharidIsVisible(bool Visible, bool Value, Entity.SCMATLASCELL_ProductRequestDetails Item)
		{
			// خرید
			Ref_SCMATLASCELL_ProductRequestDetails_DeficitSupplyIsEnable.SetVisible(Visible);
			Item.DeficitSupplyIsEnable = Value;
			// تعداد تامین کسری	
			Ref_SCMATLASCELL_ProductRequestDetails_DeficitSupplyNumber.SetVisible(Visible);
			// مبلغ حدودی خرید
			Ref_SCMATLASCELL_ProductRequestDetails_PurchasePrice.SetVisible(Visible);
			//Item.PurchasePrice = Value;
		}

		public async Task TahvilKharidIsVisible(bool Visible, bool Value, Entity.SCMATLASCELL_ProductRequestDetails Item)
		{
			// تحویل
			Ref_SCMATLASCELL_ProductRequestDetails_GoodsDeliveryIsEnable.SetVisible(Visible);
			Item.GoodsDeliveryIsEnable = Value;
		...
		public async Task GridSCMATLASCELL_ProductRequestId_817_afterrendermodal(Entity.SCMATLASCELL_ProductRequestDetails Item)
		{
			...
			if (Item.ResultingFromITIL != null)
			{
				Ref_SCMATLASCELL_ProductRequestDetails_ITILDetails.SetEntity(Item);
				Ref_SCMATLASCELL_ProductRequestDetails_ITILDetails.LoadData();
			}
		}
		...
		public async Task ResultingFromITIL_onitemselected(dynamic Selected, Entity.SCMATLASCELL_ProductRequestDetails Item)
		{

...
{
    public class Form_312Base : Form_312Peropeties
    {
    ...
    public override async Task<Result> BeforSubmit()
    {
			...
			for (int i = 0; i < _Entity.SCM_ProductRequestDetails.Count; i++)
			{
				...
				if (item.SCM_ProcurementId != null)
				{
					...
					item.ProcurementWorkAllocationDateTime = PC.GetYear(DateNow) + "/" + PC.GetMonth(DateNow).ToString("0#") + "/" + PC.GetDayOfMonth(DateNow).ToString("0#");
				}
			}
    ...
    public async Task<bool> SCM_ProductRequestDetails_editmodelsaving(object e   )
        {
            ...
            var ModelDetails = (Entity.SCM_ProductRequestDetails)e;

			// Note: شرط پُر بودن فیلد نام کالا دیزیبل
			if (ModelDetails.SCM_ProcurementId == null)
			{
				IsCancelled = true;                
				toastService.ShowError("لطفا گزینه کارشناس تدارکات را تکمیل نمایید",
					settings =>

...
{
    public class Form_510Base : Form_510Peropeties
    {
        ...
        {
            {
                ...
                // دکمه ثبت و ادامه           
                if (BtnWorkFlowId == "SequenceFlow_123ug2d")
                {
                    foreach (var Item in List)
                    {
        ...
        public async Task<bool> CheckFieldValidation(Entity.SCM_ProductRequestDetails Item)
        {
            ...
            //  // استعلام تدراکات 1
            // if(Item.SCM_ProductRequestDetails_InquiryFirst == null && Item.SCM_ProductRequestDetails_InquiryFirst.Count == 0)
            // {
            // 	IsValid = false;
            // 	toastService.ShowError("لطفا گزینه استعلام تدراکات 1 را بارگذاری نمایید.",
            // 	settings => {
            // 		settings.Timeout = 4;
            // 		settings.ShowProgressBar = true;
            // 		settings.PauseProgressOnHover = true;
            ...
            if (Item.InquiryTrueFalse.HasValue && Item.InquiryTrueFalse.Value)
            {
                ...
                if (Item.SCM_ProductRequestDetails_InquiryFirst == null || Item.SCM_ProductRequestDetails_InquiryFirst.Count == 0)
                {
                    ...
                    toastService.ShowError("لطفا فایل استعلام تدارکات 1 را انتخاب نمایید.",
                    settings =>
                    {
                        settings.Timeout = 4;
                        settings.ShowProgressBar = true;
                ...
                if (string.IsNullOrWhiteSpace(Item.ConfirmationOfTheInquiryLogistics))
                {
                    IsValid = false;

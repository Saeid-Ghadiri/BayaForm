...
{
    public class Form_432Base : Form_432Peropeties
    {
    /// <summary>
    /// آماده سازی فرم
    /// </summary>
    ...
    public override async Task<bool> FormValidator()
    {
		...
		foreach(var Item in List)
		{
        	...
        	if(Item.IsEnableNeedNegotiated == null)
        	{
				...
				toastService.ShowError("لطفا گزینه آیا نیاز به مذاکره دارد؟ را انتخاب نمایید.",
				settings => {
					settings.Timeout = 4;
    ...
    #region FunctionEvents
		public async Task <bool> SCMPETCO_ProductRequestDetails_editmodelsaving(object e   )
        {
			...
			// IsEnableNeedNegotiated - آیا نیاز به مذاکره دارد؟
			if (ModelDetail.IsEnableNeedNegotiated == null)
			{
				IsCancelled = true;
        ...
        // گزینه مذاکره مدیر عامل
		public async Task  CheckCEO_oninput(ChangeEventArgs Selected   )
        { 
            if(_Entity.SCMPETCO_ProductRequestDetails == null)
                return;
            if(string.Equals(Selected.Value.ToString(),"True")){ 
                foreach (var Item in _Entity.SCMPETCO_ProductRequestDetails)
                {
                    Item.IsEnableNeedNegotiated = true;
                }
            }
             else if (string.Equals(Selected.Value.ToString(),"False")){ 
                foreach (var Item in _Entity.SCMPETCO_ProductRequestDetails)
                {
                    Item.IsEnableNeedNegotiated = false;
                }
            }

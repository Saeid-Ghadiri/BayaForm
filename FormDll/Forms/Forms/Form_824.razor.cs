...
{
	// بخش تایید کنندگان سرپرستان و روئسا
	//public class Form_286Base : Form_286Peropeties 
	public class Form_824Base : Form_824Peropeties
	{
		// Toast  
		[Inject]
		...
		public async Task<bool> SCM_ProductRequestDetails_editmodelsaving(object e)
		{
			...
			// DM - Detail Model
			var DM = (Entity.SCM_ProductRequestDetails)e;

			// Note: شرط پُر بودن فیلد نام کالا دیزیبل
			if (DM.ProductNameText == null)
			{
				IsCancelled = true;
				toastService.ShowError("لطفا گزینه نام کالا را تکمیل نمایید",
					settings =>
            ...
            if(DM.Global_SCMRequestTypeId.HasValue && DM.Global_SCMRequestTypeId.Value.ToString() == "a9c5df1c-d99c-ef11-8354-005056a02a64")
            {
				if(!DM.ProductInventoryText.HasValue || DM.ProductInventoryText==0)
				{
					IsCancelled = true;
            ...
            if(DM.Global_SCMRequestTypeId.HasValue && DM.Global_SCMRequestTypeId.Value.ToString() == "73f6c459-d99c-ef11-8354-005056a02a64")
            {
			...
			if (DM.ProductRequestingQTY == null || DM.ProductRequestingQTY == 0)
			{
				IsCancelled = true;
			...
			// TDS فایل
			if(DM.ProductDataSheetTrueFalse.HasValue && DM.ProductDataSheetTrueFalse.Value)
			{
				if (DM.SCM_ProductRequestDetails_ProductDataSheetFile == null)
				{
					IsCancelled = true;
			...
			// ForeignMachineryProductTrueFasle - نحوه تامین کالا
			if (DM.ForeignMachineryProductTrueFasle == null)
			{
				IsCancelled = true;

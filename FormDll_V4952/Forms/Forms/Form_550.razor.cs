...
{
    public class Form_550Base : Form_550Peropeties
    {
    ...
    public override async Task<bool> FormValidator()
    {
        ...
        foreach (var Item in List)
        {
            ...
            {
                if (String.IsNullOrEmpty(Item.ProcurementConfirmedInquiryNum))
			    {
			    	IsValid = false;

			    	toastService.ShowError("لطفا گزینه استعلام مورد تایید تدارکات را تکمیل نمایید.",
			    	settings => {
			    		settings.Timeout = 4;
			    		settings.ShowProgressBar = true;
			    		settings.PauseProgressOnHover = true;
            ...
            Console.WriteLine("Log1 => " + await Utility.JSON.ToJson(Item.SCM_OS_Details_InquiryFile1.Count()));
            //
		    if (Item.InquiryIsEnable.HasValue && Item.InquiryIsEnable.Value)
		    {
		        //if(Item.SCM_OS_Details_InquiryFile1 == null) 
		        //if(Item.SCM_OS_Details_InquiryFile1 == null && Item.SCM_OS_Details_InquiryFile1.Count() == 0)
		        if(Item.SCM_OS_Details_InquiryFile1.Count() == 0)
                {
                    IsValid = false;

			    	toastService.ShowError("لطفا گزینه استعلام 1 را بارگذاری نمایید.",
			    	settings => {
			    		settings.Timeout = 4;
			    		settings.ShowProgressBar = true;
			    		settings.PauseProgressOnHover = true;
    ...
    public async Task <bool> SCM_OS_Details_editmodelsaving(object e   )
        {
            ...
            // آیا نیاز به استعلام دارد؟
            if (ModelDetail.InquiryIsEnable == null)
			{
				IsCancelled = true;
            ...
            // استعلام مورد تایید تدارکات
            if(ModelDetail.InquiryIsEnable.HasValue && ModelDetail.InquiryIsEnable.Value)
            {
                if (String.IsNullOrEmpty(ModelDetail.ProcurementConfirmedInquiryNum))
			    {
			    	IsCancelled = true;
        ...
        public async Task SCM_OS_Details_afterrendermodal(Entity.SCM_OS_Details Item   )
        {
            ...
            var DescISVisible = Ref_SCM_OS_Details_ProcurementConfirmedInquiryNum;
		    if (Item.InquiryIsEnable.HasValue && Item.InquiryIsEnable.Value)
		    {
		        UF1IsVisible.SetVisible(true); 
		        UF2IsVisible.SetVisible(true); 
		        UF3IsVisible.SetVisible(true);
                DescISVisible.SetVisible(true);
		    }
		    else
		    {
                ...
                DescISVisible.SetVisible(false);
		    }

...
{
    public class Form_998Base : Form_998Peropeties
    {
        ...
        public override async Task<bool> FormValidator()
        {
            ...
            // دکمه ثبت و ادامه           
            if ((BtnWorkFlowId == "SequenceFlow_1nclods") || (BtnWorkFlowId == "SequenceFlow_0ka8wik") || (BtnWorkFlowId == "SequenceFlow_1yhku63") || (BtnWorkFlowId == "SequenceFlow_04ukoqs"))
            {
        ...
        public async Task<bool> CheckFieldValidation(Entity.SCM_ProductRequestDetails Item)
        {
            ...
            // تعیین وضعیت باقی مانده خرید- SCM_PurchaseRemainingStatusId
            if (Item.SCM_PurchaseRemainingStatusId == null)
            {
                IsValid = false;

                toastService.ShowError("لطفا گزینه تعیین وضعیت باقی مانده خرید را تکمیل نمایید.",
                settings =>
                {
                    settings.Timeout = 4;
                    settings.ShowProgressBar = true;
        ...
        public async Task SCM_ProductRequestDetails_afterrendermodal(Entity.SCM_ProductRequestDetails Item)
        {
            ...
            if (Item.InquiryTrueFalse.HasValue && Item.InquiryTrueFalse.Value)
            {
                ...
                // اتعداد یا مقدار واگذاری کالا 
                if (Item.SCM_NumTransfersGoodsWarehouse != null)
                    Item.SCM_NumTransfersGoodsWarehouse.code = null;
                await LogisticDeliveryIsVisible(false);
                //Ref_SCM_ProductRequestDetails_SCM_NumTransfersGoodsWarehouseId.DropdownSelected = null;
            }
            else
            {
                // 

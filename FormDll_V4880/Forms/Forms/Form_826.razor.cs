...
namespace Forms.Forms
{
    ...
    public class Form_826Base : Form_826Peropeties
    {
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            if (firstRender)
            {
                // var List = _Entity.SCM........._ProductRequestDetails.ToList();
        ...
        public override async Task<bool> FormValidator()
        {
            ...
            foreach (var Item in List)
            {
                ...
                {
                    IsValid = false;
                    toastService.ShowError("لطفا از گزینه جستجو برای تکمیل اطلاعات شماران استفاده نمایید.",
                        settings =>
                        {
                            settings.Timeout = 4;
...
public async Task HavaleMasrafIsVisible(bool Visible)
{
    ...
    try
    {
        //Console.WriteLine("#Log Ref" + (Ref_SCMPETCO_ProductRequestDetails_T_Search_NotMapped == null).ToString());
...
public async Task HavaleMasrafSetShomaran(dynamic Selected, Entity.SCMPETCO_ProductRequestDetails Item)
{
    ...
    Ref_SCMPETCO_ProductRequestDetails_SH_Tahvil_DTL.LoadData();
        ...
        public async Task  T_Search_NotMapped_onitemselected(dynamic Selected ,Entity.SCMPETCO_ProductRequestDetails Item  )
        {

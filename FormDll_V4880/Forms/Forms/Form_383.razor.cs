...
{
    public class Form_383Base : Form_383Peropeties
    {
    ...
    public override async Task<bool> FormValidator()
    {
        bool IsValid = true;
            ...
            // دکمه ثبت و ادامه           
            if (BtnWorkFlowId == "SequenceFlow_1k52f5n"||BtnWorkFlowId == "SequenceFlow_0wxczmc")
            {
            ...
            // لغو کلی درخواست
            if (BtnWorkFlowId == "SequenceFlow_0c81esc" ||BtnWorkFlowId == "SequenceFlow_1n0nyyl")
            {
    ...
    public async Task<bool> CheckFieldValidation(Entity.SCM_OS_Details Item)
 		{
             ...
             //استعلام تایید شده مدیر کارخانه
                if (Item.InquiryIsEnable.HasValue && Item.InquiryIsEnable.Value)
                {
                    // FM_ConfirmedInquiryNum
                    if (Item.FM_ConfirmedInquiryNum == null)
                    {
                        IsValid = false;
                        toastService.ShowError("لطفا گزینه استعلام تایید شده مدیر کارخانه را تکمیل نمایید.",
                        settings =>
                        {
                            settings.Timeout = 4;
        ...
        var Inq7IsVisible = Ref_SCM_OS_Details_FM_ConfirmedInquiryNum;
